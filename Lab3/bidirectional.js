  ...
  Promise.all([d3.csv(ireland), d3.csv(scotland), d3.csv(england), d3.csv(wales)])
    ...
    .then(function(files) {
      ...
      function dashboard(id, countryData) {
        ...
        countryData.forEach(function(d) {d.total = d.totals.totalVacc + d.totals.peopleVacc + d.totals.peopleFully + d.totals.totalBoost;});
        ...
        function histoGram(fD) {
          ...
          var hG = {},
            ...
            margins = {top: 60, right: 0, bottom: 30, left: 0};
          ...
          //create x scale
          var x = d3.scaleBand().domain(fD.map(function(d) { return d[0];}))
            										 .range([70, width])
           										   .padding([0.2])
          ...
          function mouseover(d, i) {
            ...
            var state = countryData.filter(function(d) { return d.Country == i[0];})[0];
            ...
            var newData = Object.keys(state.totals).map(function(s) {return {type: s, totals: state.totals[s]};});
          ...
          function pieChart(pieData){
              ...
              var marginP = {width:250, height: 250};
              ...
              var pie = d3.pie().sort(null).value(function(d) {return d.totals})
              ...
              pathPie.enter()
                     ...
                     .attr("d", arc)
                  	 ...
                  	 .style("fill", function(d, i) { return pieSectionColour(d.data.type); })
                  	 .on("mouseover",mouseover)
                     .on("mouseout",mouseout);
              ...
              function mouseout(d){
                  ...
                  hG.update(countryData.map(function(d){
                      return [d.Country,d.total];}), "steelblue");
              }
          ...
          function legend(legendData){
              ...
              tr.append("td").append("svg").attr("width", '16')
                ...
                .attr("width", '16').attr("height", '16')
            		.attr("fill",function(d){ return pieSectionColour(d.type); });
              ...
              tr.append("td").attr("class",'legendFreq')
                  .text(function(d){ return d3.format(",")(d.totals);});
              ...
              leg.update = function(newData){
                  ...
                  l.select(".legendFreq").transition().duration(500).text(function(d){ return d3.format(",")(d.totals);});

                  // update the percentage column.
                  l.select(".legendPerc").transition().duration(500).text(function(d){ return getLegend(d,newData);});        
              }

              function getLegend(d,aD){ // Utility function to compute percentage.
                  return d3.format(".2%")(d.totals/d3.sum(aD.map(function(v){ return v.totals; })));
              }
        ...
        var totalValues = ['totalVacc','peopleVacc','peopleFully', "totalBoost"].map(function(d){ 
            return {type:d, totals: d3.sum(countryData.map(function(t){ return t.totals[d];}))}; 
        });    
       
        // calculate total frequency by state for all segments
        var sF = countryData.map(function(d) {return [d.Country, d.total]; })
        ...
        var hG = histoGram(sF),pC = pieChart(totalValues), leg= legend(totalValues);
      }
